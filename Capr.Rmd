...
loc <- getwd()
data <- read.csv(paste(loc,"/ckme136/exchange.csv",sep=""), na.strings = c("","NA"), stringsAsFactors = F, header = T)
...
# Taking the monthly mean value of the exchange rate
data<- aggregate(Exchange_Rate ~ DATE, data, mean)

data <- subset(data, DATE<= "Dec 2017" )

# exchange rate of USDCAD (Final Data for analysis )
...
loc <- getwd()
oil_price_data <- read.csv(paste(loc,"/ckme136/Oil_prices.csv",sep=""), na.strings = c("","NA"), stringsAsFactors = F, header = T)
...
oil_price_data <- oil_price_data[,1:2]
oil_price_data<- oil_price_data[1:8772,]

# Changing Column Names
names<- c("DATE", "oil_price")
colnames(oil_price_data)<-names

# Changing to numeric
oil_price_data$oil_price<- as.numeric(oil_price_data$oil_price)

# Converting to Date
oil_price_data$DATE<-as.Date(oil_price_data$DATE, format = "%d-%b-%y")
oil_price_data$DATE<-as.yearmon(oil_price_data$DATE)

# Taking the monthly mean value of the oil prices
oil_price_data<- aggregate(oil_price ~ DATE, oil_price_data, mean)

# subsetting data from Mar 1988
oil_price_data <- subset(oil_price_data, DATE >= "Mar 1988" & DATE <= "Dec 2017")


# checking NA's
sum(is.na(oil_price_data))
head(oil_price_data)
```
```{r}
...
```{r}
# -------------------------------------------------------------------------------------------------------------
# Merging Attributes data to create our final data we name as CED ( Combined Exchange Data)

CED <- Reduce(function(x, y) merge(x, y, all=TRUE), list(exchange_rate_data,oil_price_data, import_price_index_us, interest_rate_canada, 
...
cor(CED$Manu_Index_US[114:358], CED$Non_Manu_Index_US[114:358])
plot(CED$Manu_Index_US[114:358], CED$Non_Manu_Index_US[114:358])
...
cor(CED$Imp_Pr_Index_US[11:358], CED$ppi_comm_US[11:358])
plot(CED$Imp_Pr_Index_US[11:358], CED$ppi_comm_US[11:358])
CED<- CED[-3]
...
out_treat<- function(f){
  a <- round(quantile(f,.25)- 1.5* IQR(f))
  b <- round(quantile(f,.75)+ 1.5* IQR(f))
  percentile<- ecdf(f)
  e<- percentile(a)
  d<- percentile(b)
...
suffStat <- list(C = cor(CED_Scaled), n = nrow(CED_Scaled))
...
# converting the pcalg network to bnlearn 
pc.CED.bn<- as.bn(pc.CED)
plot(pc.CED.bn, main = " Network converted from pcalg to bnlearn")

```
```{r}
# The causal network from pcalg shows us a couple of new causal links "oil_price -> "unemp_CA ", "unemp_rate_US"-> "Int_US" which are considered to be  incorpoarted into the bnlearn network  hes  . Interest Differential is known to be the foremost driver in exchange rates, hence "Int_Diff"-> "Exchange_Rate" is also incorporated .
whitelist = data.frame( from = c("oil_price","unemp_rate_US","Int_Diff"), to = c("unemp_CA","Int_US", "Exchange_Rate"))
...
# We see that the model learned by hill climbing method( Cond Prob = 11.52(may vary on different runs)) is more accurate than the model created by data definitions ( Cond Prob = 8.19 (may vary on different runs) ) . The actual Cond Prob = 10.8 
```
...
# We see that the model learned by hill climbing method( Cond Prob = 8.4 ( may vary on different runs)) is more accurate than the model created by data definitions ( Cond Prob = 4.1 (may vary on different runs)). The actual Cond Prob = 11.11
```
```{r}
...
Cond_Prob_oil_Int
# We see that the model learned by hill climbing method( Cond Prob = 14.45 (may vary on different runs)) is more accurate than the model created by data definitions ( Cond Prob = 11.46(may vary on different runs)) . The actual Cond Prob = 14.28
```
